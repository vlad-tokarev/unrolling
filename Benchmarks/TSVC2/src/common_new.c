...
void set_3d_array(real_t arr[LEN_3D][LEN_3D][LEN_3D], real_t value, int stride);

enum {SET1D_RECIP_IDX = -1, SET1D_RECIP_IDX_SQ = -2};

real_t sum1d(real_t arr[LEN_1D]);
real_t sum2d(real_t arr[LEN_2D][LEN_2D]);
real_t sum3d(real_t arr[LEN_3D][LEN_3D][LEN_3D]);
...
void set_3d_array(real_t arr[LEN_3D][LEN_3D][LEN_3D], real_t value, int stride)
{
    for (int i = 0; i < LEN_3D; i++) {
        for (int j =0; j < LEN_3D; j++)
        set_2d_array(arr[i][j], LEN_3D, value, stride);
    }
}
